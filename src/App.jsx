...
import {
  ...
  LogIn, Lock, MessageCircle, Send, UserCircle, CheckSquare, Square, Trash2, Landmark, Download
} from "lucide-react";
...
async function downloadSyndicatePdf(pool) {
  ...
  doc.setFontSize(10);
  doc.setTextColor(80);
  doc.text(`Total shares: ${totalShares}  ·  Price per share: ${money(pool.pricePerShare)}`, marginX, y);
  y += 10;
  ...
  doc.text("Shares", 92, y);
  doc.text("Share %", 116, y);
  doc.text("Winnings", 145, y);
  ...
  pool.participants.forEach((p) => {
    ...
    doc.text(String(p.shares), 92, y);
    doc.text(`${share.toFixed(2)}%`, 116, y);
    doc.text(money(winnings), 145, y);
    doc.text(p.paid ? "Paid" : "Unpaid", 176, y);
    y += 7;
  });
...
async function loadMemberships(userId) {
  const { data, error } = await supabase
    .from("participants")
    .select("shares, syndicate:syndicates!inner(code,name,jackpot,status,actual_winnings,deleted_at)")
    .eq("user_id", userId)
...
async function addParticipant(code, { name, shares, userId }) {
  const { data, error } = await supabase
    .from("participants")
    .insert({ syndicate_code: code, name, shares, amount: 0, fee: 0, paid: false, user_id: userId })
    .select()
    .single();
  if (error) throw error;
  return data.id;
...
function JoinPool({ session, initialPool, onBack, onDone }) {
  ...
  async function handleConfirm() {
    ...
    setError("");
    try {
      const newId = await addParticipant(pool.code, { name: pname.trim(), shares, userId: session.user.id });
    ...
    <Screen>
      ...
      <div className="flex-1 px-6 pb-10">
        ...
        <div className="flex items-center justify-between mb-2.5">
          ...
          <span className="text-[11px] uppercase tracking-wide text-[#6B7A76]">Potential winnings</span>
        </div>
          ...
          {pool.participants.map((p) => {
            ...
            return (
              <div key={p.id} className="bg-white rounded-xl px-4 py-3.5 flex items-center gap-3">
                ...
                <div className="font-[JetBrains_Mono] text-[14px] text-[#10201D] font-medium shrink-0">{money(winnings)}</div>
              </div>
            );
          })}
        </div>
