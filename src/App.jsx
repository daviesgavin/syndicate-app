...
import {
  Ticket, Users, Copy, Check, ArrowRight, ArrowLeft, PlusCircle, ImageOff, LogOut, HardDrive, MapPin, Mail,
...
async function requestPayout(participantId, code) {
  const { data: syn } = await supabase.from("syndicates").select("rolled_forward_to").eq("code", code).single();
  ...
  const { data, error } = await supabase.from("payout_requests").insert({ participant_id: participantId, syndicate_code: code }).select().single();
  if (error) throw error;
  await logActivity(code, `A participant requested a cash-out payout.`);
  return data.id;
}
...
async function submitPayoutBankDetails(requestId, { bankName, accountName, bsb, accountNumber }) {
  const { error } = await supabase.from("payout_requests").update({
    bank_name: bankName, account_name: accountName, bsb, account_number: accountNumber,
  }).eq("id", requestId);
  if (error) throw error;
}
...
async function markPayoutPaid(requestId, participantId, code) {
  const { data: p } = await supabase.from("participants").select("name, amount").eq("id", participantId).single();
  ...
  await logActivity(code, `${p?.name || "A participant"} was paid out ${money(p?.amount || 0)} and left the syndicate.`);
}
...
function AdminScreen({ session, onBack, onOpenSyndicate }) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    const [s, list, v, loc] = await Promise.all([loadAdminStats(), loadAllSyndicates({ includeDeleted: showDeleted }), loadVisitorStats(), loadVisitorLocations()]);
    setStats(s);
...
function JoinPool({ session, initialPool, onBack, onDone }) {
  ...
  async function handleConfirm() {
    ...
    setError("");
    try {
      // re-check against the LATEST deadline in case it changed since this page was opened
      const latest = await loadPool(pool.code);
      if (latest && latest.entryDeadline && new Date(latest.entryDeadline).getTime() <= Date.now()) {
        setPool(latest);
        setError("Entries have just closed for this syndicate.");
        setSaving(false);
        return;
      }
      ...
      if (isRlsError) {
        setError("This couldn't be completed — the entry deadline may have just changed. Try going back and checking the countdown, then try again.");
      } else {
  ...
  async function handleRequestCashOut() {
    ...
    try {
      ...
      if (bankName || accountName || bsb || accountNumber) {
        await submitPayoutBankDetails(id, { bankName, accountName, bsb, accountNumber });
      }
  ...
  return (
    ...
    <Screen>
      ...
      <div className="flex-1 px-6 pb-10">
        ...
        {myParticipant && myPayoutRequest === null && !pool.rolledForwardTo && (
          <button onClick={() => setShowCashOut(true)} className="w-full flex items-center gap-3 bg-[#C1473A]/8 border border-[#C1473A]/25 rounded-xl px-4 py-3 mb-5 text-left">
            ...
            <div className="flex-1">
              ...
              <div className="text-[11.5px] text-[#8A6A15]">Take your current share ({money(myParticipant.amount)}) and leave this syndicate</div>
            </div>
          </button>
        )}
      ...
      <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/40" onClick={() => setShowCashOut(false)}>
        <div className="w-full max-w-[430px] bg-[#F7F2E7] rounded-t-3xl p-6 pb-8 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <p className="text-[13px] text-[#6B7A76] mb-4">
            You'll leave this syndicate once the organiser pays out your current share ({myParticipant ? money(myParticipant.amount) : ""}). Adding your bank details is optional but makes it easier for the organiser to pay you — only they can see them.
          </p>
              ...
              {payoutRequests.map((req) => (
                <div key={req.id} className="bg-white rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="font-[JetBrains_Mono] text-[15px] font-bold text-[#10201D]">{money(req.participant.amount)}</div>
                  </div>
