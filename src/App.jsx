...
function Dashboard({ session, code, onBack, onSignIn, isAdmin }) {
  ...
  async function handleMergeGroup(group) {
    const keepId = keepChoice[group[0].userId] || group[0].id;
    const keepName = group.find((p) => p.id === keepId)?.name || group[0].name;
    setMergingUserId(group[0].userId);
    try {
      ...
      <div className="flex-1 px-6 pb-10">
        ...
        {duplicateList.length > 0 && (
          <button onClick={() => setShowDuplicates(true)} className="w-full flex items-center gap-3 bg-[#C9982E]/12 border border-[#C9982E]/30 rounded-xl px-4 py-3 mb-4 text-left">
            ...
            <div className="flex-1">
              <div className="text-[13px] font-medium text-[#10201D]">{duplicateList.length} duplicate {duplicateList.length === 1 ? "entry" : "entries"} found</div>
              <div className="text-[11.5px] text-[#8A6A15]">Same account joined more than once — tap to merge</div>
            </div>
                ...
                return (
                  <div key={uid} className="bg-white rounded-2xl p-4">
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#F0EBDC]">
                      ...
                      <Button full={false} onClick={() => handleMergeGroup(group)} disabled={mergingUserId === uid} icon={mergingUserId === uid ? Loader2 : Check}>
