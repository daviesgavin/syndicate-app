...
async function createPool({ name, organiser, pricePerShare, jackpot, drawDate }) {
...
function getMyPools() {
  try {
    return JSON.parse(localStorage.getItem("myPools") || "[]");
  } catch (e) {
    return [];
  }
}
function addMyPool(code, name) {
  const list = getMyPools().filter((p) => p.code !== code);
  localStorage.setItem("myPools", JSON.stringify([{ code, name }, ...list]));
}
...
function Home({ onCreate, onJoin, myPools }) {
  return (
    <Screen dark>
      <div className="flex-1 flex flex-col px-6 pt-16 pb-10">
        ...
        {myPools.length > 0 && (
          <div className="mt-auto">
            <div className="text-[12px] uppercase tracking-wide text-[#6B7A76] mb-2.5">Your syndicates</div>
...
function CreatePool({ onBack, onCreated }) {
  ...
  async function handleCreate() {
    ...
    setError("");
    try {
      const code = await createPool({ name: name.trim(), organiser: organiser.trim(), pricePerShare: Number(price), jackpot: Number(jackpot), drawDate });
      addMyPool(code, name.trim());
      onCreated(code);
    } catch (e) {
      setError("Something went wrong creating the syndicate. Check your Supabase setup and try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Screen>
      ...
      <div className="flex-1 px-6 pt-2 pb-10 flex flex-col">
        ...
        <div className="text-[13.5px] text-[#6B7A76] mb-8">Organised by {pool.organiser} · {money(pool.pricePerShare)} per share</div>
        <div className="space-y-3">
          <Button icon={ArrowRight} onClick={onJoin}>Reserve my shares</Button>
          <Button variant="ghost" icon={Eye} onClick={onView}>View tickets &amp; results</Button>
        </div>
      </div>
    </Screen>
  );
...
function JoinPool({ initialPool, onBack, onDone }) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setError("Couldn't save your shares. Try again.");
    } finally {
      setSaving(false);
    }
  }
    ...
    return (
      <Screen>
        ...
        <div className="flex-1 px-6 pt-2 pb-8 flex flex-col">
          <div className="flex items-center gap-2 text-[#6B7A76] text-[13px] mb-1"><Trophy size={14} className="text-[#C9982E]" />Jackpot estimate</div>
          <div className="font-[Fraunces] text-[34px] text-[#10201D] font-medium mb-6">{money(pool.jackpot)}</div>
          <TicketCard stub={
          ...
          <div className="mt-6"><Button onClick={() => setStep("name")} icon={ArrowRight}>Continue</Button></div>
        </div>
      </Screen>
    );
  }
    ...
    return (
      <Screen>
        ...
        <div className="flex-1 px-6 pt-2 pb-8">
          ...
          {error && <div className="flex items-center gap-2 text-[#C1473A] text-[13px] mb-4"><AlertCircle size={15} /> {error}</div>}
          <Button onClick={handleConfirm} disabled={!pname.trim() || saving} icon={saving ? Loader2 : ArrowRight}>{saving ? "Saving…" : "Confirm my shares"}</Button>
        </div>
      </Screen>
    );
  }
  ...
  return (
    <Screen>
      ...
      <div className="flex-1 px-6 pt-6 pb-10 flex flex-col items-center">
        ...
        <p className="text-[12.5px] text-[#8A968F] text-center mt-6 leading-relaxed">Keep your code — {pool.code} — to check results after the draw.</p>
  ...
  return (
    <Screen>
      ...
      <div className="flex-1 px-6 pb-10">
        ...
        <div className="text-[12px] uppercase tracking-wide text-[#6B7A76] mb-2.5">Participants</div>
        <div className="space-y-2">
...
function Dashboard({ code, onBack }) {
  const [pool, setPool] = useState(null);
...
export default function App() {
  ...
  useEffect(() => { setMyPools(getMyPools()); }, [route]);
  ...
  const [subRoute, setSubRoute] = useState(null); // 'landing' | 'join' | 'view' | 'code'
  ...
  if (route.name === "dashboard") return <Dashboard code={route.code} onBack={goHome} />;
  ...
  if (subRoute === "create") {
    return <CreatePool onBack={() => setSubRoute(null)} onCreated={(code) => goDashboard(code)} />;
  }
  return <Home onCreate={() => setSubRoute("create")} onJoin={() => setSubRoute("code")} myPools={myPools} />;
}
