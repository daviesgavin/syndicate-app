...
async function mergeParticipantGroup(ids, keepId, keepName) {
  const { data, error } = await supabase.from("participants").select("id, amount").in("id", ids);
  if (error) throw error;
...
async function mergeParticipantAmount(participantId, additionalAmount) {
  const { data, error } = await supabase.from("participants").select("amount").eq("id", participantId).single();
  if (error) throw error;
  const newAmount = Number(data.amount || 0) + additionalAmount;
  ...
  const { error: updateError } = await supabase.from("participants").update({ amount: newAmount, paid: false }).eq("id", participantId);
  if (updateError) throw updateError;
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const newId = existing
        ? await mergeParticipantAmount(existing.id, numericAmount)
...
function Dashboard({ session, code, onBack, onSignIn, isAdmin }) {
  ...
  async function handleManualMerge() {
    ...
    try {
      await mergeParticipantGroup(manualSelectedIds, manualKeepId, keepName);
  ...
  async function handleMergeGroup(group, groupKey) {
    ...
    try {
      await mergeParticipantGroup(group.map((p) => p.id), keepId, keepName);
      await refresh();
    } finally {
