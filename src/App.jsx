...
import {
  Ticket, Users, Copy, Check, ArrowRight, ArrowLeft, PlusCircle, ImageOff, LogOut, HardDrive, MapPin, Mail, Star,
...
async function loadOwnedPools(ownerId) {
  const { data, error } = await supabase
    .from("syndicates")
    .select("code,name,rolled_forward_to")
    .eq("owner_id", ownerId)
    .is("deleted_at", null)
    .order("created_at", { ascending: false });
  if (error) return [];
  return data.map((s) => ({ code: s.code, name: s.name, rolledForwardTo: s.rolled_forward_to }));
}

async function loadMemberships(userId) {
  const { data, error } = await supabase
    .from("participants")
    .select("amount, syndicate:syndicates!inner(code,name,jackpot,status,actual_winnings,deleted_at,rolled_forward_to)")
    .eq("user_id", userId)
...
function AdminScreen({ session, onBack, onOpenSyndicate }) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    const [s, list, v, loc, reviews] = await Promise.all([loadAdminStats(), loadAllSyndicates({ includeDeleted: showDeleted }), loadVisitorStats(), loadVisitorLocations(), loadPendingReviews()]);
    setStats(s);
...
function WinnersTicker({ winners }) {
  ...
  function formatEntry(w) {
    const location = w.win_city
      ? (w.win_region ? `${w.win_city}, ${w.win_region}` : w.win_city)
      : (COUNTRIES[w.country]?.name || "Australia");
