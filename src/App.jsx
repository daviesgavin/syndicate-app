...
async function loadOwnedPools(ownerId) {
  const { data, error } = await supabase
    .from("syndicates")
    .select("code,name,rolled_forward_to,entry_deadline,status,actual_winnings")
    .eq("owner_id", ownerId)
    .is("deleted_at", null)
    .order("created_at", { ascending: false });
  if (error) return [];
...
// Counts remaining (unpaid-out) participants per syndicate code, used to tell whether a
// drawn syndicate has had every member paid out and closed off.
async function loadParticipantCounts(codes) {
  if (!codes || codes.length === 0) return {};
  const { data, error } = await supabase.from("participants").select("syndicate_code").in("syndicate_code", codes);
  if (error || !data) return {};
  const counts = {};
  data.forEach((r) => { counts[r.syndicate_code] = (counts[r.syndicate_code] || 0) + 1; });
  return counts;
}
...
async function loadMemberships(userId) {
  const { data, error } = await supabase
    .from("participants")
    .select("amount, syndicate:syndicates!inner(code,name,jackpot,status,actual_winnings,deleted_at,rolled_forward_to,entry_deadline)")
    .eq("user_id", userId)
  ...
  useEffect(() => {
    ...
    Promise.all([loadOwnedPools(session.user.id), loadMemberships(session.user.id), loadProfile(session.user.id)]).then(
      async ([owned, member, prof]) => {
        ...
        // everyone can be flagged as closed, rather than just left showing "Closed"
        const codes = [...new Set([...owned.map((p) => p.code), ...member.map((m) => m.code)])];
        const counts = await loadParticipantCounts(codes);
        setMyPools(owned.map((p) => ({ ...p, participantCount: counts[p.code] || 0 })));
        setMemberships(member.map((m) => ({ ...m, participantCount: counts[m.code] || 0 })));
        setProfile(prof);
        setLoading(false);
      }
    );
      ...
      <div className="flex-1 flex flex-col px-6 pt-16 pb-10 relative">
        ...
        {session && !loading && (
          <div className="space-y-6">
            {myPools.length > 0 && (() => {
              const activePools = myPools.filter((p) => !p.rolledForwardTo);
              const completedPools = myPools.filter((p) => p.rolledForwardTo);
              return (
                <div>
                      ...
                      <div className="space-y-2 mt-2">
                        {completedPools.map((p) => (
                          <a key={p.code} href={`#/dashboard/${p.code}`} className="w-full flex items-center justify-between bg-white/5 hover:bg-white/10 rounded-xl px-4 py-3.5 text-left opacity-60">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-[11px] font-bold shrink-0 text-[#C1473A]">Rolled Over</span>
                              <ChevronRight size={17} className="text-[#7C8C88]" />
                            </div>
                          </a>
                        ))}
            ...
            <div>
              ...
              {memberships.length === 0 ? (
                ...
                const activeMemberships = memberships.filter((m) => !m.rolledForwardTo);
                const completedMemberships = memberships.filter((m) => m.rolledForwardTo);
                return (
                  <>
                    <div className="space-y-2">
...
function Dashboard({ session, code, onBack, onSignIn, isAdmin }) {
  ...
  // Unpaid entries first so the organiser can see who still owes money without scrolling
  // past everyone who's already settled up; stable sort keeps each group's original order.
  const sortedParticipants = [...pool.participants].sort((a, b) => Number(a.paid) - Number(b.paid));
