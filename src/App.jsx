...
async function sendMessage(code, { userId, senderName, senderAvatar, body }) {
  const { error } = await supabase.from("messages").insert({ syndicate_code: code, user_id: userId, sender_name: senderName, sender_avatar: senderAvatar, body });
  if (error) throw error;
}
...
function ChatRoom({ session, code, poolName, onBack, onSignIn }) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToMessages(code, (msg) => setMessages((prev) => [...prev, msg]));
    return unsub;
  }, [code]);
  ...
  async function handleSend() {
    ...
    try {
      ...
      // best-effort; message will simply not appear
    } finally {
      setSending(false);
    }
  }
      ...
      <div className="px-4 pb-4 pt-2 sticky bottom-0 bg-[#F7F2E7]">
        {session ? (
          <div className="flex gap-2">
            ...
            />
            <button onClick={handleSend} disabled={!body.trim() || sending} className="w-12 h-12 rounded-xl bg-[#2F6F5E] text-white flex items-center justify-center disabled:opacity-40 shrink-0">
