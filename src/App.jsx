...
import {
  ...
  LogIn, Lock, MessageCircle, Send, UserCircle, CheckSquare, Square, Trash2, Landmark, Download, PlusSquare, Smartphone
} from "lucide-react";
...
async function addParticipant(code, { name, amount, userId }) {
  const { data, error } = await supabase
    .from("participants")
    .insert({ syndicate_code: code, name, amount, fee: 0, paid: false, user_id: userId })
    .select()
    .single();
  if (error) throw error;
  return data.id;
  ...
  if (carryMembers) {
    for (const p of oldPool.participants) {
      ...
      await supabase.from("participants").insert({
        syndicate_code: newCode, name: p.name, amount: p.amount, user_id: p.userId,
        fee: 0, paid: false,
      });
    }
  }
      ...
      <Screen>
        ...
        <div className="flex-1 px-6 pt-10 flex flex-col items-center text-center">
          ...
          <div className="bg-white rounded-xl px-4 py-3 mt-6 text-[12.5px] text-[#6B7A76] leading-relaxed">
            The email will arrive from <strong className="text-[#3E5652]">"Supabase Auth"</strong> — that's the service we use to send it, not spam.
          </div>
        </div>
      </Screen>
    );
...
const LEGAL_CONTENT = {
  privacy: {
    ...
    sections: [
      ...
      { h: "Contact", p: ["daviesgavin@hotmail.com"] },
    ],
  },
  terms: {
...
function Home({ session, onCreate, onJoin, onSignIn, onSignOut, onProfile, onLegal }) {
  const [profile, setProfile] = useState(null);
...
function CreateSuccessScreen({ session, sessionId, onDone, onError }) {
  const [status, setStatus] = useState("checking"); // checking | creating | error
  ...
  useEffect(() => {
    async function run() {
      ...
      try {
        ...
        setStatus("error");
        setMessage(e.message || "Something went wrong finishing setup.");
      }
    }
    run();
  }, [sessionId]);
...
function JoinPool({ session, initialPool, onBack, onDone }) {
  ...
  const myPct = projectedTotal > 0 ? (amount / projectedTotal) * 100 : 0;
  const myWinnings = (myPct / 100) * pool.jackpot;

  function adjustAmount(delta) {
    setAmount((a) => Math.max(1, Math.round((a + delta) * 100) / 100));
  }
  ...
  async function handleConfirm() {
    ...
    setError("");
    try {
      const newId = await addParticipant(pool.code, { name: pname.trim(), amount, userId: session.user.id });
          ...
          <TicketCard stub={
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 flex items-center justify-center gap-1">
                <span className="font-[Fraunces] text-[28px] text-[#10201D] font-medium">$</span>
                <input
                  type="number"
                  min="1"
                  step="1"
                  ...
                  className="font-[Fraunces] text-[32px] font-medium text-[#10201D] leading-none w-24 text-center bg-transparent focus:outline-none"
                />
              </div>
            ...
            <div className="text-[11px] uppercase tracking-wide text-[#6B7A76] text-center mt-2">any amount from $1</div>
          </TicketCard>
            ...
            return (
              <div key={p.id} className="bg-white rounded-xl px-4 py-3.5 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[12px] text-[#8A968F]">{money(p.amount)} · {pct(share)}</div>
                </div>
