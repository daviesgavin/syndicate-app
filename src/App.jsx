...
import {
  ...
  LogIn, Lock, MessageCircle, Send, UserCircle, CheckSquare, Square, Trash2, Landmark, Download, PlusSquare, Smartphone, Pencil, UserX, BookOpen
} from "lucide-react";
...
function AdminScreen({ session, onBack, onOpenSyndicate }) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    const [s, list] = await Promise.all([loadAdminStats(), loadAllSyndicates({ includeDeleted: showDeleted })]);
    setStats(s);
    setSyndicates(list);
    setLoading(false);
  }, [showDeleted]);
          ...
          {syndicates.map((s) => (
            <div key={s.code} className="bg-white rounded-xl px-4 py-3 flex items-center justify-between">
              ...
              {s.deleted_at ? (
                ...
                <ChevronRight size={16} className="text-[#6B7A76] shrink-0 ml-2" />
              )}
            </div>
          ))}
        </div>
  ...
  useEffect(() => {
    async function run() {
      ...
      try {
        ...
        const newCode = await rolloverSyndicate(draft.oldPool, draft.options);
        localStorage.removeItem("pendingRollover");
        onDone(newCode);
      } catch (e) {
        setStatus("error");
        setMessage(e.message || "Something went wrong finishing the rollover.");
      }
    }
    run();
  }, [sessionId]);
...
export default function App() {
  ...
  if (route.name === "dashboard") {
    return <Dashboard session={session} code={route.code} onBack={goHome} onSignIn={requestSignIn} isAdmin={isAdmin} />;
  }
