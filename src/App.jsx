...
import {
  ...
  LogIn, Lock, MessageCircle, Send, UserCircle
} from "lucide-react";
...
async function loadPool(code) {
  ...
  return {
    ...
    participants: (parts || []).map((p) => ({
      id: p.id, name: p.name, shares: p.shares, paid: p.paid, paidAt: p.paid_at, userId: p.user_id,
...
async function loadMemberships(userId) {
  const { data, error } = await supabase
    .from("participants")
    .select("shares, syndicate:syndicates(code,name,jackpot,status,actual_winnings)")
    .eq("user_id", userId)
...
async function addParticipant(code, { name, shares, userId }) {
  const { error } = await supabase.from("participants").insert({ syndicate_code: code, name, shares, amount: 0, fee: 0, paid: false, user_id: userId });
  if (error) throw error;
}
...
async function rolloverSyndicate(oldPool, { jackpot, pricePerShare, drawDate, entryDeadline, carryMembers, ownerId }) {
  ...
  if (carryMembers) {
    for (const p of oldPool.participants) {
      ...
      await supabase.from("participants").insert({
        syndicate_code: newCode, name: p.name, shares: p.shares, user_id: p.userId,
        amount: 0, fee: 0, paid: false,
      });
    }
  }
  return newCode;
...
function DeadlineBadge({ deadline }) {
  ...
  const closed = diff <= 0;
  return (
    <div className="flex items-center gap-2 rounded-xl px-3.5 py-2.5 mb-4 bg-[#C1473A]/8">
...
function AuthScreen({ onBack }) {
  ...
  async function handleSignIn() {
    ...
    try {
      ...
      setError(e.message === "Invalid login credentials" ? "Wrong email or password." : (e.message || "Couldn't sign in."));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleForgot() {
    ...
    try {
      ...
      setError(e.message || "Couldn't send that reset email.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Screen>
      ...
      <div className="flex-1 px-6 pt-4">
        ...
        {mode === "signup" && <p className="text-[12.5px] text-[#8A968F] -mt-2 mb-4">Use at least 6 characters.</p>}

        {error && <div className="flex items-center gap-2 text-[#C1473A] text-[13px] mb-4"><AlertCircle size={15} /> {error}</div>}
        {message && <div className="flex items-center gap-2 text-[#2F6F5E] text-[13px] mb-4"><Check size={15} /> {message}</div>}

        {mode === "signin" && (
          <>
            <Button onClick={handleSignIn} disabled={!email.trim() || !password || loading} icon={loading ? Loader2 : ArrowRight}>{loading ? "Signing in…" : "Sign in"}</Button>
            <div className="flex justify-between mt-4 text-[13px]">
              <button onClick={() => switchMode("signup")} className="text-[#2F6F5E] underline">Create account</button>
              <button onClick={() => switchMode("forgot")} className="text-[#6B7A76] underline">Forgot password?</button>
            </div>
          </>
        )}
        ...
        {mode === "signup" && (
          <>
            <Button onClick={handleSignUp} disabled={!email.trim() || password.length < 6 || loading} icon={loading ? Loader2 : ArrowRight}>{loading ? "Creating…" : "Create account"}</Button>
            <button onClick={() => switchMode("signin")} className="text-[13px] text-[#2F6F5E] underline mt-4">Already have an account? Sign in</button>
          </>
        )}

        {mode === "forgot" && (
          <>
            <Button onClick={handleForgot} disabled={!email.trim() || loading} icon={loading ? Loader2 : ArrowRight}>{loading ? "Sending…" : "Send reset link"}</Button>
            <button onClick={() => switchMode("signin")} className="text-[13px] text-[#2F6F5E] underline mt-4">Back to sign in</button>
          </>
        )}
      </div>
    </Screen>
...
function ResetPasswordScreen({ onDone }) {
  ...
  return (
    <Screen>
      ...
      <div className="flex-1 px-6 pt-4">
        <p className="text-[#5B6B67] text-[14.5px] leading-relaxed mb-6">Choose a new password for your account.</p>
        <Field label="New password">
          <input className={inputCls} type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} autoFocus />
        </Field>
        {error && <div className="flex items-center gap-2 text-[#C1473A] text-[13px] mb-4"><AlertCircle size={15} /> {error}</div>}
        <Button onClick={handleSave} disabled={password.length < 6 || saving} icon={saving ? Loader2 : Check}>{saving ? "Saving…" : "Save new password"}</Button>
      </div>
    </Screen>
  );
}
...
function Home({ session, onCreate, onJoin, onSignIn, onSignOut, onProfile }) {
  const [profile, setProfile] = useState(null);
  ...
  return (
    <Screen dark>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-8 right-8 flex gap-2 rotate-[8deg] opacity-80">
          <div className="w-8 h-8 rounded-full bg-[#F7F2E7] shadow-lg flex items-center justify-center">
            <span className="font-[Fraunces] text-[11px] font-semibold text-[#2F6F5E]">7</span>
          </div>
          <div className="w-7 h-7 rounded-full bg-[#F7F2E7] shadow-lg flex items-center justify-center mt-3 -ml-1">
            <span className="font-[Fraunces] text-[10px] font-semibold text-[#C1473A]">23</span>
          </div>
        </div>
        <div className="absolute top-24 right-16 w-6 h-6 rounded-full bg-[#F7F2E7] shadow-lg flex items-center justify-center rotate-[-6deg] opacity-70">
          <span className="font-[Fraunces] text-[9px] font-semibold text-[#C9982E]">41</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col px-6 pt-16 pb-10 relative">
        ...
        <p className="text-[#9FB0AC] text-[15px] leading-relaxed mb-8 max-w-[320px]">
                  ...
                  {memberships.map((m) => (
                    <a key={m.code} href={`#/j/${m.code}`} className="w-full flex items-center justify-between bg-white/5 hover:bg-white/10 rounded-xl px-4 py-3.5 text-left">
                      <div>
                        ...
                        <div className="text-[#7C8C88] text-[12px] mt-0.5">{m.shares} share{m.shares === 1 ? "" : "s"} · {m.status === "drawn" ? "Drawn" : "Open"}</div>
                      </div>
                      <ChevronRight size={17} className="text-[#7C8C88]" />
                    </a>
                  ))}
...
function JoinPool({ session, initialPool, onBack, onDone }) {
  ...
  async function handleConfirm() {
    ...
    setError("");
    try {
      await addParticipant(pool.code, { name: pname.trim(), shares, userId: session.user.id });
  ...
  return (
    <Screen>
      ...
      <div className="flex-1 px-6 pb-10">
        ...
        {pool.entryDeadline && <DeadlineBadge deadline={pool.entryDeadline} />}
        <Button variant="ghost" icon={MessageCircle} onClick={onChat}>Syndicate chat</Button>
        <div className="text-[12px] uppercase tracking-wide text-[#6B7A76] mb-2.5 mt-5">Ticket photos</div>
            ...
            return (
              <div key={p.id} className="bg-white rounded-xl px-4 py-3.5 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[12px] text-[#8A968F]">{p.shares} share{p.shares === 1 ? "" : "s"} · {pct(share)}</div>
                </div>
...
function Dashboard({ session, code, onBack, onSignIn }) {
  ...
  const [rolloverPrice, setRolloverPrice] = useState("");
  ...
  async function handleRollover() {
    ...
    setRolloverError("");
    try {
      const newCode = await rolloverSyndicate(pool, {
        jackpot: Number(rolloverJackpot),
        pricePerShare: Number(rolloverPrice || pool.pricePerShare),
      ...
      window.location.hash = `/dashboard/${newCode}`;
    } catch (e) {
      setRolloverError("Something went wrong creating the new syndicate.");
    } finally {
        ...
        {pool.status !== "drawn" ? (
          ...
          <>
            ...
            {pool.rolledForwardTo ? (
              ...
              <button onClick={() => { setRolloverPrice(String(pool.pricePerShare)); setShowRollover(true); }} className="w-full flex items-center justify-between bg-[#C9982E]/12 text-[#8A6A15] rounded-2xl px-5 py-4 mb-5 border border-[#C9982E]/30">
      ...
      {showRollover && (
        <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/40" onClick={() => setShowRollover(false)}>
          <div className="w-full max-w-[430px] bg-[#F7F2E7] rounded-t-3xl p-6 pb-8 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[13px] text-[#6B7A76] mb-4">Starts a new "{pool.name}" syndicate with {money(pool.actualWinnings)} noted as rolled over.</p>
            ...
            <Field label="Price per share"><input className={inputCls} inputMode="decimal" value={rolloverPrice} onChange={(e) => setRolloverPrice(e.target.value.replace(/[^0-9.]/g, ""))} /></Field>
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={handleRollover} disabled={!rolloverJackpot || rolloverSaving} icon={rolloverSaving ? Loader2 : ArrowRight}>{rolloverSaving ? "Creating…" : "Create new syndicate"}</Button>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, newSession) => {
      ...
      if (event === "PASSWORD_RECOVERY") setSubRoute("resetpassword");
    });
  ...
  if (subRoute === "resetpassword") {
    return <ResetPasswordScreen onDone={() => setSubRoute(null)} />;
  }

  if (route.name === "dashboard") {
    return <Dashboard session={session} code={route.code} onBack={goHome} onSignIn={requestSignIn} />;
  }

  if (subRoute === "signin") {
    return <AuthScreen onBack={() => setSubRoute(null)} />;
  }
  ...
  if (route.name === "code-landing") {
    ...
    if (subRoute === "join" && foundPool) {
      if (!session) return <AuthScreen onBack={() => setSubRoute("landing")} />;
      return <JoinPool session={session} initialPool={foundPool} onBack={() => setSubRoute("landing")} onDone={goHome} />;
    }
    if (subRoute === "view" && foundPool) {
      return <ViewPool code={foundPool.code} onBack={() => setSubRoute("landing")} onChat={() => setSubRoute("chat")} />;
    }
    if (subRoute === "chat" && foundPool) {
      return <ChatRoom session={session} code={foundPool.code} poolName={foundPool.name} onBack={() => setSubRoute("landing")} onSignIn={requestSignIn} />;
    }
    return (
  ...
  if (subRoute === "create") {
    if (!session) return <AuthScreen onBack={() => setSubRoute(null)} />;
