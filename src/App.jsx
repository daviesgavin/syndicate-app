...
async function loadOwnedPools(ownerId) {
  const { data, error } = await supabase
    .from("syndicates")
    .select("code,name,rolled_forward_to,entry_deadline")
    .eq("owner_id", ownerId)
    .is("deleted_at", null)
    .order("created_at", { ascending: false });
  if (error) return [];
  return data.map((s) => ({ code: s.code, name: s.name, rolledForwardTo: s.rolled_forward_to, entryDeadline: s.entry_deadline }));
}
...
function MiniCountdown({ deadline }) {
  ...
  const flashing = diff > 0 && diff < 60 * 60 * 1000; // under 60 minutes left
  return (
    <span className={`text-[11px] font-[JetBrains_Mono] font-medium text-[#C1473A] shrink-0 ${flashing ? "animate-pulse" : ""}`}>
      {formatCompactCountdown(diff)}
    </span>
  );
}
